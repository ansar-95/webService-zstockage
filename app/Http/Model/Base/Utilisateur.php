...
abstract class Utilisateur implements ActiveRecordInterface
{
    ...
    /**
     * The value for the numutilisateur field.
     *
     * @var        int
     */
    ...
    /**
     * Get the [numutilisateur] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [numutilisateur] column.
     *
     * @param int $v New value
     * @return $this|\App\Http\Model\Utilisateur The current object (for fluent API support)
     */
    public function setNumutilisateur($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->numutilisateur !== $v) {
            $this->numutilisateur = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_NUMUTILISATEUR] = true;
        }
    ...
    public function setType($v)
    {
        ...
        if ($this->type !== $v) {
            $this->type = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_TYPE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UtilisateurTableMap::translateFieldName('Numutilisateur', TableMap::TYPE_PHPNAME, $indexType)];
            $this->numutilisateur = (null !== $col) ? (int) $col : null;
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UtilisateurTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
            $this->password = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UtilisateurTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = UtilisateurTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_NUMUTILISATEUR)) {
            $modifiedColumns[':p' . $index++]  = 'numUtilisateur';
        }
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_TYPE)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'numUtilisateur':
                        $stmt->bindValue($identifier, $this->numutilisateur, PDO::PARAM_INT);
                        break;
                    case 'raisonSociale':
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setNumutilisateur($arr[$keys[0]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_NUMUTILISATEUR)) {
            $criteria->add(UtilisateurTableMap::COL_NUMUTILISATEUR, $this->numutilisateur);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_RAISONSOCIALE)) {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_TYPE)) {
            $criteria->add(UtilisateurTableMap::COL_TYPE, $this->type);
        }
    ...
    {
        $validPk = null !== $this->getNumutilisateur();
    ...
    /**
     * Generic method to set the primary key (numutilisateur column).
     *
    ...
    {
        return null === $this->getNumutilisateur();
    }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setNumutilisateur(NULL); // this is a auto-increment column, so set to default value
        }
    }
